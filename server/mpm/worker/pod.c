...
AP_DECLARE(int) ap_worker_pod_check(ap_worker_pod_t *pod)
{
    ...
    if (rc == 1) {
        ...
        case GRACEFUL_CHAR:
            return AP_GRACEFUL;
        case RECONFIGURE_CHAR:
        	return AP_RECONFIGURE;
        }
    }
    return AP_NORESTART;
}
...
static apr_status_t pod_signal_internal(ap_worker_pod_t *pod, int graceful)
{
     ...
     */
    char char_of_death = GRACEFUL_CHAR;
    switch(graceful){
    case 0:
    	char_of_death = RESTART_CHAR;
    	break;
    case 1:
    	char_of_death = GRACEFUL_CHAR;
    	break;
    case 2:
    	char_of_death = RECONFIGURE_CHAR;
    	break;
    }
    /*
     * *********** end amiya 20140217 *************
...
/*
 * amiya: 'graceful' indicates which POD char to send
 * with our addition, 0: restart 1: graceful 2: reconfigure
 */
AP_DECLARE(void) ap_worker_pod_killpg(ap_worker_pod_t *pod, int num, int graceful)
{
     ...
     * ********** begin amiya 20140217 ************
     */
    ap_log_error(APLOG_MARK, APLOG_ALERT, rv, NULL, APLOGNO(0999)
                         "writing %d pod chars from killpg. graceful: %d",num, graceful);
    /*
     * ********** end amiya 20140217 *************
     */
