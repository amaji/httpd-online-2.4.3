 ...
 *
 * *********** amiya **********
 * apache uses max_daemons_limit as a means to optimize scans in perform_idle_server_mainttenance
 * typically it is assumed that max_daemons_limit <= ap_daemons_limit and
 * ap_daemons_limit <= server_limit
 *
 * we are breaking this assumption so that max_daemons_limit can be >= ap_daemons_limit
 * additional assumption is that server is not restarted between reconfigurations
 * TODO: CHECK: (it is possible that apache resets this number after restart, else the last mxc configuration
 * before restart takes over.)
 *
 */
...
/*
 * ********** begin amiya 20140220 *************
 */

#ifndef HTTPD_RUNNING_CONFIG_FILE
#define HTTPD_RUNNING_CONFIG_FILE "/tmp/httpd_running_config.conf"
#endif
...
running_params *rp;
/*
 * TODO: Change this to a mmap file so that the file needs to be opened only once
 */
void read_running_params(running_params *p)
{
	int fd = open(HTTPD_RUNNING_CONFIG_FILE,O_RDONLY);
	/*
	 * File format is assumed to be a follows:
	 * MXC=1234
	 * KAT=10
	 */
	if(fd != -1) {
		char buf[64];
		char param[8], value[8];
		int i,j,n = 0;

		//TODO: add bounds check for array indices
		while((n = read(fd,buf,64)) >0) {
			...
			while(i < n) {
				...
				if(param[0] == 'M' || param[0] == 'm')
					p->maxclients = atoi(value);
				else if(param[0] == 'K' || param[0] == 'k')
					p->keepalivetimeout = atoi(value);
			}
		}
		close(fd);
	}
...
static int find_active_servers(void)
{
	int i, num=0;

	for (i = 0; i < retained->max_daemons_limit; ++i) {
		if (ap_scoreboard_image->parent[i].pid != 0 && ap_scoreboard_image->parent[i].quiescing == 0) {
			num++;
		}
	}
	return num;
	...
	for (i = 0; i < retained->max_daemons_limit; ++i) {
		if (ap_scoreboard_image->parent[i].pid != 0) {
			 ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
					 "pid: %d -> quiescing: %d -> generation: %d",ap_scoreboard_image->parent[i].pid, ap_scoreboard_image->parent[i].quiescing, ap_scoreboard_image->parent[i].generation);
		}
	}
	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
						 "retained->max_daemons_limit: %d ap_daemons_limit: %d ---------------------------------",retained->max_daemons_limit,ap_daemons_limit);
	return;
}

/*
 * This is a copy of worker_check_config but only contains checks related to MaxRequestWorkers
 * (MaxClients) and KeepaliveTimeout
 * The purpose of copying this function is to keep all the global variables consistent with
 * what Httpd expects
 */
static int worker_runtime_check_config(void)//apr_pool_t *p, apr_pool_t *plog,
                               //apr_pool_t *ptemp, server_rec *s)
{
    ...
    if (max_workers < threads_per_child) {
        if (startup) {
            ...
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00315)
    ...
    if (max_workers % threads_per_child) {
        ...
        if (startup) {
            ...
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00317)
    ...
    if (ap_daemons_limit > server_limit) {
        if (startup) {
            ...
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00319)
    ...
    if (ap_daemons_to_start > ap_daemons_limit) {
        if (startup) {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00320)
                         "WARNING: StartServers of %d is larger than what MaxRequestWorkers can support, "
                         "Reducing to %d.", ap_daemons_to_start, ap_daemons_limit);
        } else {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00321)
                         "StartServers of %d is larger than what MaxRequestWorkers can support,"
            			" Reducing to %d.", ap_daemons_to_start, ap_daemons_limit);
        }
        ap_daemons_to_start = ap_daemons_limit;
    }

    if (min_spare_threads > max_workers) {
        if (startup) {
            ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00322)
                         "WARNING: MinSpareThreads of %d is gretaer than MaxRequestWorkers, "
                         "reducing it to %d", min_spare_threads, max_workers);
        } else {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00323)
                         "MinSpareThreads of %d is gretaer than MaxRequestWorkers, "
            			 "reducing it to %d", min_spare_threads, max_workers);
        }
        min_spare_threads = (max_workers - threads_per_child) > 0 ? (max_workers - threads_per_child) : max_workers;
    }

    if (max_spare_threads > max_workers) {
    	if (startup) {
    		ap_log_error(APLOG_MARK, APLOG_WARNING | APLOG_STARTUP, 0, NULL, APLOGNO(00322)
    				"WARNING: MinSpareThreads of %d is gretaer than MaxRequestWorkers, "
    				"reducing it to %d", max_spare_threads, max_workers);
    	} else {
    		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(00323)
    				"MaxSpareThreads of %d is gretaer than MaxRequestWorkers, "
    				"reducing it to %d", max_spare_threads, max_workers);
    	}
...
void update_global_running_config(void)
{
	ap_scoreboard_image->global->running_maxclients = (ap_daemons_limit*threads_per_child);
	if(ap_server_conf)
		ap_scoreboard_image->global->running_keepalivetimeout = apr_time_sec(ap_server_conf->keep_alive_timeout);
}

static void master_update_running_params(void)
{
	max_workers = rp->maxclients;
	if(ap_server_conf)
		ap_server_conf->keep_alive_timeout = apr_time_from_sec(rp->keepalivetimeout);
	else
		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
				"CRITICAL: Cannot find server_config for this server.");

	worker_runtime_check_config();	//this would verify value of maxclients and
									//update ap_daemons_limit

	update_global_running_config();	//update entries in sb->global_score. pass values to children

	//ignoring updating config_tree for now. it would require considerable effort to find a
	//directive under a module and then update it.
	//
	//worker_module->MaxRequestWorkers is already handled by ap_daemons_limit
	//server->keep_alive_timeout: hopefully updating the server_rec handles this

	int num_active_servers = find_active_servers();
	int new_workers_limit = ap_daemons_limit;

	if(num_active_servers > new_workers_limit)
	{
		int children_to_kill = num_active_servers - new_workers_limit;
		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
						"Active servers: %d. Preparing to send %d POD chars to children",num_active_servers,children_to_kill);
		//ap_worker_pod_killpg(pod, children_to_kill, 1);	//graceful
		for(int n=0; n<children_to_kill; n++)
			ap_worker_pod_signal(pod, TRUE);
		retained->idle_spawn_rate = 1;

		ap_worker_pod_killpg(pod, (num_active_servers - children_to_kill), 2); //reconfigure
	}
	else if(num_active_servers < new_workers_limit)
	{
		int children_to_create = new_workers_limit - num_active_servers;
		ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
				"MaxClients updated. We shall now create %d new children",children_to_create);
		//we won't really create new children.
		//we just update global vars and let perform_idle_server_maintenance take over
		//otherwise it is possible that server is idle but we create new children
		retained->maxclients_reported = 0;
		retained->near_maxclients_reported = 0;

		ap_worker_pod_killpg(pod, num_active_servers, 2); //reconfigure
	}
	else	// num_active_servers == new_workers_limit
		ap_worker_pod_killpg(pod, num_active_servers, 2); //reconfigure
...
static void child_update_running_params(void)
{
	/*
	 * We are not synchronizing access to scoreboard since these params are not critical.
	 * Also, child reads scoreboard only after parent updates it and signals.
	 */
	int mxc, kat;
	mxc = ap_scoreboard_image->global->running_maxclients;
	kat = ap_scoreboard_image->global->running_keepalivetimeout;
	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
			"pid: %d Read params from sb, mxc: %d kat: %d",ap_my_pid,mxc,kat);

	/*
	 * We do not sync access to ap_server_conf->keep_alive_timeout
	 * Worker will read it after next request anyway.
	 */
	ap_server_conf->keep_alive_timeout = apr_time_from_sec(kat);
...
static void wakeup_listener(void)
{
    ...
    if (!listener_os_thread) {
    	 ...
    	 *
    	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
    						 "pid %d: Reached dummy block in wakeup_listener()",ap_my_pid);
    	/*
    	 * ************* end amiya *************
    	 */
        return;
    ...
    int rv = ap_queue_info_term(worker_queue_info);
    /*
     ************** amiya debug message *************
     */
    //ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
    //		"pid %d: ap_queue_info_term() returned: %d",ap_my_pid,rv);
    /*
     * ************* end amiya *************
     */
...
static void signal_threads(int mode)
{
    if (terminate_mode == mode) {
    	 ...
    	 *
    	ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL, APLOGNO(0999)
    			"pid %d: Found terminate_mode == mode = %d",ap_my_pid,mode);
    	/*
    	 * ************* end amiya *************
    	 */
        return;
...
static int volatile reconfigure_pending;	// amiya 20140217
...
{
	//printf("yay!! reconfigure() called with signal %d\n", sig);
	ap_log_error(APLOG_MARK, APLOG_ALERT, errno, ap_server_conf, APLOGNO(09999)
	                     "yay!! says pid %d reconfigure() called with signal %d", getpid(), sig);
	///TODO:
	// put check to see if this a a child process if so return without doing anything

	//if (reconfigure_pending == 1) { //AM: do i need this check??
	        /* Probably not an error - don't bother reporting it */
	//        return;
	//    }
	reconfigure_pending = 1;

	// write an if-stmt to decide on whether this is the master
	// if master update scoreboard and create/kill children else ignore
}
/*
 * ************* end amiya 20140217 **************
 */
...
static void set_signals(void)
{
     ...
     * *********** begin amiya 20140217 **********
     */
#ifdef AP_SIG_RECONFIGURE
    sigemptyset(&sa.sa_mask);
    sa.sa_flags = 0;

    sa.sa_handler = reconfigure;
    if (sigaction(AP_SIG_RECONFIGURE, &sa, NULL) < 0)
    	ap_log_error(APLOG_MARK, APLOG_WARNING, errno, ap_server_conf, APLOGNO(09999)
    			"sigaction(SIGUSR2)");
#endif
    /*
     * ********** end amiya 20140217 **********
...
#else  /*if_defined NO_USE_SIGACTION*/
    if (!one_process) {
#ifdef SIGXCPU
     ...
     */
#ifdef AP_SIG_RECONFIGURE
    apr_signal(AP_SIG_RECONFIGURE, reconfigure);
#endif
    /*
     * ************ end amiya 20140217 ***********
     */
...
static void * APR_THREAD_FUNC listener_thread(apr_thread_t *thd, void * dummy)
{
    ...
    while (1) {
         ...
         */
        if(requests_this_child % 1024 == 0)	//every 1K request print current KAT for debugging
        	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL, APLOGNO(0999)
        		"Current KAT is: %d",apr_time_sec(ap_server_conf->keep_alive_timeout));
...
static void join_workers(apr_thread_t *listener, apr_thread_t **threads)
{
   ...
   //     			APLOGNO(09999) "Pid %d: Listener not dead yet. iter: %d!! restart_pending: %d",getpid(),iter,restart_pending);
        	/************* end **********/
        ...
        if (iter >= 10) {
            ap_log_error(APLOG_MARK, APLOG_WARNING, 0, ap_server_conf, APLOGNO(00276)
                         "the listener thread didn't exit. Forcing ungraceful shutdown.");
            /*
             * ********** begin amiya 20140219 **********
             */
            workers_may_exit = 1;
            ap_queue_interrupt_all(worker_queue);
     ...
     */
//    ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL,
//    		APLOGNO(09999) "Pid %d: finished killing listener in join_workers!! listener_may_exit: %d workers_may_exit: %d",getpid(),listener_may_exit,workers_may_exit);
    /************* end **********/
...
static void child_main(int child_num_arg)
{
    ...
    if (one_process) {
        ...
        while (1) {
             ...
             * ********** begin amiya 20140217 ***********
             */
            if (rv == AP_RECONFIGURE) {
            	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL,
            	                         APLOGNO(09999) "Whoa .. I (pid %d) found a reconfigure char over the POD!!",getpid());
            	//place all code to update global vars/state variables etc. here
            	//if needed create a new function
            	child_update_running_params();
            	sleep(1);	//let other processes consume the pod chars
            				//it is not critical how long we sleep
            	continue;
            }
            /*
             * ********** end amiya 20140217 ***********
            ...
            if (rv == AP_NORESTART) {
            	 ...
            	 */
            	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL,
            			APLOGNO(09999) "Hmmmm .. I (pid %d) found a shutdown char over the POD!!",getpid());
            ...
            if (rv == AP_GRACEFUL || rv == AP_RESTART) {
            	 ...
            	 */
            	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL,
            			APLOGNO(09999) "Fine .. I (pid %d) found a graceful char over the POD!!",getpid());
                 ...
                 */
                ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL,
                		APLOGNO(09999) "Pid %d: after join_start_threads and signal_threads!! mpm_state: %d",getpid(),mpm_state);
                /************* end **********/
         ...
         * added amiya
         */
        ap_scoreboard_image->parent[child_num_arg].quiescing = 1;  //if someone is checking outside if I am an active children
        															//should be set when listener_thread() exits.
        															//but just in case it could not be woken.
//        ap_log_error(APLOG_MARK, APLOG_WARNING, 0, NULL,
//        		APLOGNO(09999) "Pid %d: before join_workers!! shutdown_pending: %d restart_pending: %d",getpid(),shutdown_pending,restart_pending);
        /************* end **********/
        join_workers(ts->listener, threads);
        /*
         * added amiya
         */
        ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL,
        		APLOGNO(09999) "Pid %d: after join_workers!! shutdown_pending: %d restart_pending: %d",getpid(),shutdown_pending,restart_pending);
        /************* end **********/
    }
...
static int make_child(server_rec *s, int slot)
{
    ...
    if (!pid) {
        ...
        /*
         * ********** begin amiya 20140217 ************
         * overwrite SIGUSR2 handling in child so that it doesn't try to reconfigure()
         */
        apr_signal(AP_SIG_RECONFIGURE, dummy_signal_handler);
        /*
         * ********** end amiya 20140217 ************
         */
        child_main(slot);
...
static void startup_children(int number_to_start)
{
     ...
     */
    int sb_index = (ap_daemons_limit > retained->max_daemons_limit) ? ap_daemons_limit : retained->max_daemons_limit;

    for (i = 0; number_to_start && i < sb_index/*ap_daemons_limit*/; ++i) {
    // ********* end amiya ********
        if (ap_scoreboard_image->parent[i].pid != 0) {
            continue;
        }
...
static void perform_idle_server_maintenance(void)
{
    ...
    /*
     * Added by amiya for debugging
     *
    static int idle_server_loops = 0;

    idle_server_loops++;
    if(idle_server_loops % 20 == 0)
    	print_process_list();
    /*
     * end amiya
     */
     ...
     * ********** begin amiya 20140223 **********
     *
     * we are changing the for loop limit to retained->max_daemons_limit
     * with reconfiguration max_daemons_limit can be >= ap_daemons_limit
     */
    int sb_limit = (retained->max_daemons_limit > ap_daemons_limit) ? retained->max_daemons_limit : ap_daemons_limit;

    for (i = 0; i < sb_limit/*ap_daemons_limit*/; ++i) {
    /*
     * ********** end amiya ***********
     */
    ...
    if (idle_thread_count > max_spare_threads) {
        ...
        if (free_length == 0) { /* scoreboard is full, can't fork */
        	...
        	/*
        	 * ******** begin Amiya 20140425 *********
        	 */
        	if (active_thread_count < ap_daemons_limit * threads_per_child)
        	/* with online httpd free_list can be > 1 even though maxclients reached */
        	/* this is because max sb indices are not strictly equal to ap_daemons_limit */
        	{
        	/*
        	 * ********* end Amiya 20140425 *********
			 ...
			 */
        		 ...
        		 retained->idle_spawn_rate = 1;	//reset spawning process
        	}
        	/*
        	 * ******** end Amiya 20140425 *********
        	 */
...
static void server_main_loop(int remaining_children_to_start)
{
    ...
    while (!restart_pending && !shutdown_pending) {
        ...
        /*
         * ********** begin amiya 20140217 ************
         * main processing logic of reconfigure()
         */
        if(reconfigure_pending)
        {	//a reconfigure signal was delivered while we were waiting
        	// process it now

        	read_running_params(rp);
        	ap_log_error(APLOG_MARK, APLOG_ALERT, 0, NULL, APLOGNO(0999)
        				"Read values from running_config. MXC: %d KAT: %d",rp->maxclients, rp->keepalivetimeout);

        	master_update_running_params();
        ...
        if (pid.pid != -1) {
            ...
            if (child_slot >= 0) {
                ...
                /*
                 * ****** edited by amiya 20140223 ******
                 */
                else if (remaining_children_to_start
                    && child_slot < retained->max_daemons_limit/*ap_daemons_limit*/) {
...
static int worker_run(apr_pool_t *_pconf, apr_pool_t *plog, server_rec *s)
{
     ...
     * ********** begin amiya 20140220 *************
     */
    rp = (running_params *)(apr_pcalloc(s->process->pool,sizeof(running_params)));
    if(rp == NULL)
    {
    	ap_log_error(APLOG_MARK, APLOG_CRIT, 0, ap_server_conf, APLOGNO(0999)
    	                "Could not allocate running_params");
    	mpm_state = AP_MPMQ_STOPPING;
    	return DONE;
    }
    /*
    ...
    if (shutdown_pending && !retained->is_graceful) {
        ...
        do {
            ...
            active_children = 0;
            /*
             * ********* edited amiya 20140223 *********
             */
            for (index = 0; index < retained->max_daemons_limit/*ap_daemons_limit*/; ++index) {
